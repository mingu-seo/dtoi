...
function directChange() {
	$("input[cst_name='direct']").val($("#direct").val());
}
function getOrderby() {
	$("input[cst_name='orderby']").val($("#orderby").val());
	$("input[cst_name='direct']").val($("#direct").val());
}
function moveWrite() {
...
${reqPage }/${totalPage } 
<select id="orderby">
	<option value="cst_regdate" <c:if test="${param.orderby == 'cst_regdate' }">selected</c:if>>가입일</option>
	<option value="cst_name" <c:if test="${param.orderby == 'cst_name' }">selected</c:if>>이름</option>
	<option value="id" <c:if test="${param.orderby == 'cst_id' }">selected</c:if>>아이디</option>
</select>
...
<c:forEach var="vo" items="${list}">
	<tr>
		<td>${vo.cst_no }</td>
		<td>${vo.cst_name }</td>
		<td><a href="detail.do?cst_no=${vo.cst_no }">${vo.cst_id }</a></td>
		<td>${vo.cst_tel }</td>
